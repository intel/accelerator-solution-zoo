...
#define MAX_WQ_SZ 128
#define MAX_USE_CPUS  128
...
struct percpu_tasks {
	...
	union {
                struct task *single_task; //single tasks array
                struct batch_task *batch_task; //batch tasks array
        };
	DECLARE_BITMAP(task_bm, MAX_WQ_SZ);
};
...
int dsa_memcpy_vector_with_tasks(struct dsa_context *ctx, struct task tasks[],  int nb_bufs);
