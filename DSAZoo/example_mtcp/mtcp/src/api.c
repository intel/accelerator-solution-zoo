...
/*----------------------------------------------------------------------------*/
struct virtqueue* get_virtq_rx(mctx_t mctx)
{
        if(mctx == NULL) {
                return NULL;
        }
        return mctx->qctx.rx_virtqueue;
}

struct virtqueue* get_virtq_tx(mctx_t mctx)
{
        if(mctx == NULL) {
                return NULL;
        }
        return mctx->qctx.tx_virtqueue;
}

/*----------------------------------------------------------------------------*/
uint64_t rcv_sc_num, rcv_fl_num;

static int CopyToUser(mtcp_manager_t mtcp, tcp_stream *cur_stream, char *buf, int len)
{
	struct tcp_recv_vars *rcvvar = cur_stream->rcvvar;
	uint32_t prev_rcv_wnd;
	int copylen = len;
	struct tcp_ring_buffer* rb = rcvvar->rcvbuf;
	int toff = rb->size / MAX_PKT_SIZE;
	//copylen = MIN(rcvvar->rcvbuf->merged_len, len);
	if (copylen <= 0) {
		errno = EAGAIN;
		return -1;
	...
	//if(rb->tail_offset == rb->head_offset) {
	if(rb->last_len == 0) {
		rcv_fl_num++;
		//printf("current recv-ring is empty\n");
		errno = EAGAIN;
                return -1;
	}

	rcv_sc_num++;
	//printf("recv sucess num:%ld, fail num:%ld\n", rcv_sc_num, rcv_fl_num);
	
	/* Copy data to user buffer and remove it from receiving buffer */
	memcpy(buf, rcvvar->rcvbuf->head + rb->head_offset*MAX_PKT_SIZE, copylen);
...
/*----------------------------------------------------------------------------*/
static int copy_to_virtq(mctx_t mctx, tcp_stream *cur_stream, char *buf, int len)
{
        struct tcp_recv_vars *rcvvar = cur_stream->rcvvar;
        uint32_t prev_rcv_wnd;
        int copylen = len;
	...
	//copylen = MIN(rcvvar->rcvbuf->merged_len, len);
        ...
        struct virtqueue* vq_rx = mctx->qctx.rx_virtqueue;
        if(vq_rx == NULL) {
                printf("Invalid virtio rx queueu!\n");
                return -1;
        }

        /* Copy data to user buffer and remove it from receiving buffer */
        //memcpy(tmp_buff, rb->head + rb->head_offset*MAX_PKT_SIZE, copylen);

	ret = virtqueue_add_buf2(rb->head + rb->head_offset*MAX_PKT_SIZE, copylen, vq_rx);
        if(ret < 0) {
                errno = EAGAIN;
                printf("virtqueue en size:%d\n", ret);
                return -1;
        }
...
mtcp_recv(mctx_t mctx, int sockid, char *buf, size_t len, int flags)
{
	...
	socket->socktype = MTCP_SOCK_STREAM;
	//socket->stream->state = TCP_ST_ESTABLISHED;
	cur_stream = socket->stream;
	if(cur_stream == NULL) {
		errno = ENOTCONN;
		return -1;
        ...
        if(cur_stream->state < 1 || cur_stream->state > 6) {
		errno = EAGAIN;
                return -1;
	}
	...
	//no buffer in rcv buff ring
        /*
	if(rcvvar->rcvbuf->last_len < 1) {
                errno = EAGAIN;
                return -1;
        }
	*/
	...
	*/
	...
	switch (flags) {
	case 0:
		ret = copy_to_virtq(mctx, cur_stream, buf, len); // CopyToUser(mtcp, cur_stream, buf, len);
		break;
	case MSG_PEEK:
...
/*----------------------------------------------------------------------------*/
static inline int
CopyFromUser_dsa(mctx_t mctx, tcp_stream *cur_stream, const char *buf, int len)
{
        struct tcp_send_vars *sndvar = cur_stream->sndvar;
        uint16_t idx = 0;
        int sndlen;
	...
	if (sndlen < len) {
        ...
        //ret = SBPut(mtcp->rbm_snd, sndvar->sndbuf, buf, sndlen);
        ret = SBPut_dsa(mctx, mtcp->rbm_snd, sndvar->sndbuf, buf, sndlen);
        assert(ret == sndlen);
        sndvar->snd_wnd = sndvar->sndbuf->size - sndvar->sndbuf->len;
	if (ret <= 0) {
                TRACE_ERROR("SBPut failed. reason: %d (sndlen: %u, len: %u\n",
...
/*----------------------------------------------------------------------------*/
static inline int
CopyFromUser_vector(mctx_t mctx, tcp_stream *cur_stream, const struct iovec *iov, int numIOV)
{
       struct tcp_send_vars *sndvar = cur_stream->sndvar;
       int sndlen , total_len= 0;
       int ret, i;
       ...
       sndlen = MIN((int)sndvar->snd_wnd, total_len);
       //if (sndlen <= 0) {
       if (sndlen < total_len) {
               errno = EAGAIN;
               return -1;
       ...
       ret = SBPut_vector(mctx, mtcp->rbm_snd, sndvar->sndbuf, iov, numIOV, total_len);
       assert(ret == sndlen);
       sndvar->snd_wnd = sndvar->sndbuf->size - sndvar->sndbuf->len;
       //printf("snd_wnd is %d\n", sndvar->snd_wnd);
       if (ret <= 0) {
...
mtcp_write(mctx_t mctx, int sockid, const char *buf, size_t len)
{
	...
	sfifo_in(mctx->psfifo, buf, len);
        struct entry te = {.pdata = NULL, .size = 0,};
        sfifo_out(mctx->psfifo, &te);
        char *obuf = te.pdata;
        size_t olen = te.size;
	...
	ret = CopyFromUser(mtcp, cur_stream, obuf, olen);

	SBUF_UNLOCK(&sndvar->write_lock);

	//printf("copy from user return %d\n", ret);
...
/*----------------------------------------------------------------------------*/
static inline int 
CopyFromUser_dsa_async(mctx_t mctx, tcp_stream *cur_stream, const char *buf, int len)
{
	struct tcp_send_vars *sndvar = cur_stream->sndvar;
	int sndlen;
	int ret;
	...
	sndlen = MIN((int)sndvar->snd_wnd, len);
	if (sndlen < len) { //full sendbuf at one time will have good performance
		errno = EAGAIN;
		return -1;
	}
	...
	//save some context that will be used for modify sendbuf variables when dsa copy complete
	mtcp_async_save_context(mctx, cur_stream, sndvar, len);
	ret = SBPut_async(mctx, mtcp->rbm_snd, sndvar->sndbuf, buf, sndlen); //copy data to sendbuf use dsa
	assert(ret == sndlen);
	sndvar->snd_wnd = sndvar->sndbuf->size - sndvar->sndbuf->len;
	//printf("snd_wnd is %d\n", sndvar->snd_wnd);
	if (ret <= 0) {
...
mctx_t mctx_tmp;
tcp_stream *cur_stream_tmp;
struct tcp_send_vars *sndvar_tmp;
uint32_t tail_off_tmp;
size_t len_tmp;

void mtcp_async_save_context(mctx_t arg1, tcp_stream *arg2, struct tcp_send_vars *arg3, size_t arg4)
{
	mctx_tmp = arg1;
	cur_stream_tmp = arg2;
	sndvar_tmp = arg3;
...
void mtcp_async_get_context(mctx_t *arg1, tcp_stream **arg2, struct tcp_send_vars **arg3, size_t *arg4)
{
	*arg1 = mctx_tmp;
	*arg2 = cur_stream_tmp;
	*arg3 = sndvar_tmp;
...
bool dsa_running = false;
static int once = 0;

//*----------------------------------------------------------------------------*/
ssize_t
mtcp_write_async(mctx_t mctx, int sockid, const char *buf, size_t len)
{
	//if dsa state is running, the lastest write process is not complete
	if(dsa_running == true)
		return -1;
...
#endif
	//copy data use dsa
	ret = CopyFromUser_dsa_async(mctx, cur_stream, obuf, olen);
	if(ret == len) {
        	dsa_running = true; //if dsa submit success, dsa state is running
	}
	SBUF_UNLOCK(&sndvar->write_lock);
	return ret;
}
...
mtcp_writev(mctx_t mctx, int sockid, const struct iovec *iov, int numIOV)
{
	...
	*/
	ret = CopyFromUser_vector(mctx, cur_stream, iov, numIOV);
	to_write = ret;
	SBUF_UNLOCK(&sndvar->write_lock);
