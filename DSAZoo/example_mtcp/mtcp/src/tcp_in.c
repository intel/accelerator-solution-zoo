		...
		uint32_t cur_ts, uint8_t *payload, uint32_t seq, int payloadlen)
{
	...
	if(tv_cur.tv_sec - tv_prv.tv_sec > 2){
		float sec_passed = tv_cur.tv_sec - tv_prv.tv_sec + ((float)(tv_cur.tv_usec - tv_prv.tv_usec))/1000000;
		float pps = tcp_rcv_num / sec_passed;
		printf("TCP recved PPS:%.2f\n", pps);
...
{
	//hard code to let stack go ahead
	tcph->syn = 0;
	if (tcph->syn) {
		 ...
		 uint32_t cur_ts, const int ifidx, const struct iphdr *iph, int ip_len)
{
	...
	//printf("tcph syn=%u, ack=%u, seq=%d, ack_seq=%d \n", tcph->syn, tcph->ack, tcph->seq, tcph->ack_seq);
	...
	//hard code to let tcp state established
	cur_stream->state = TCP_ST_ESTABLISHED;
