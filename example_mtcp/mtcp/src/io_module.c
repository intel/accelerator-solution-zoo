...
SetNetEnv(char *dev_name_list, char *port_stat_list)
{
	...
	if (current_iomodule_func == &ps_module_func) {
		...
		snprintf(mem_channels, sizeof(mem_channels), "%d", CONFIG.num_mem_ch);
		...
		snprintf(socket_mem_str, sizeof(socket_mem_str), "%d", socket_mem);
#if 0
		...
		uint16_t port_id = 0;
		printf("ww dev name list:%s\n", dev_name_list);
		const char *port_name = dev_name_list;
		if (rte_eth_dev_get_port_by_name(port_name, &port_id) == 0) {
			eidx = CONFIG.eths_num++;
			strncpy(CONFIG.eths[eidx].dev_name, port_name, 128);
			CONFIG.eths[eidx].dev_name[128 - 1] = '\0';
	...
	for (i = 0; i < CONFIG.eths_num; ++i) {
		...
		if (strstr(port_stat_list, CONFIG.eths[i].dev_name))
