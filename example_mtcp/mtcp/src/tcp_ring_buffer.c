	   ...
	   void* data, uint32_t len, uint32_t cur_seq)
{
	...
	int merged = 0;
	int toff = buff->size / MAX_PKT_SIZE;

	if (len <= 0)
		return 0;
	
	/*MAX_PKT_SIZE is enough for any sigle tcp pkt, always to use this size*/
	len = MAX_PKT_SIZE;
	...
	//if tail get end of ring, rewind it
	if(buff->tail_offset >= toff){
		buff->tail_offset = 0;
	}

	putx = buff->tail_offset * MAX_PKT_SIZE;
#if 0
	...
	memcpy(buff->head + putx, data, len);
#endif
	//update tail and last_len
	//buff->tail_offset = (buff->tail_offset + 1) % toff;
	if(buff->tail_offset >= buff->head_offset) {
		buff->last_len = buff->tail_offset - buff->head_offset;
		if(buff->tail_offset + 1 == toff) 
			buff->last_len += 1;
		buff->tail_offset = (buff->tail_offset + 1) % toff;
	}
	else {
		buff->tail_offset = (buff->tail_offset + 1) % toff;
		buff->last_len = toff - (buff->head_offset - buff->tail_offset);
	}
	rbp_num++;
	
	//printf("[put] rbp-num:%ld, last_len=%d, tail:%d, head:%d\n",rbp_num, buff->last_len, buff->tail_offset, buff->head_offset);
...
RBRemove(rb_manager_t rbm, struct tcp_ring_buffer* buff, size_t len, int option)
{
	...
	*/
	...
	//update tail and last_len
	/*
        buff->head_offset = (buff->head_offset + 1) % toff ;
        if(buff->tail_offset >= buff->head_offset) {
                buff->last_len = buff->tail_offset - buff->head_offset;
        }
        else {
	...
	*/
	if(buff->tail_offset > buff->head_offset) {
		...
		buff->last_len = toff - (buff->head_offset - buff->tail_offset);
		if(buff->head_offset + 1 == toff){
			buff->head_offset = 0;
		}
		else{
			buff->head_offset = (buff->head_offset + 1) % toff;
		}
		if(buff->last_len > 0) buff->last_len -= 1;
	}

        //printf("[out] last_len=%d, tail:%d, head:%d\n", buff->last_len, buff->tail_offset, buff->head_offset);
