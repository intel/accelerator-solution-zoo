...
int sndbuf_size_auto_match = 8192;
...
static char port_list[MAX_OPTLINE_LEN] = 	{'\0'};
static char port_stat_list[MAX_OPTLINE_LEN] = 	{'\0'};
...
{
	dev_name_list[1024 - 1] = '\0';
	strncpy(port_list, dev_name_list, 1024);
	port_list[1024 - 1] = '\0';
}
/*----------------------------------------------------------------------------*/
static inline void
...
{
	dev_name_list[1024 - 1] = '\0';
	strncpy(port_stat_list, dev_name_list, 1024);
	port_stat_list[1024 - 1] = '\0';
}
/*----------------------------------------------------------------------------*/
static int 
ParseConfiguration(char *line)
{
	...
	if (strcmp(p, "num_cores") == 0) {
		...
		printf("CONFIG.num_cores=%d, num_cpus=%d\n", CONFIG.num_cores, num_cpus);
		...
		CONFIG.local_ip[19] = '\0';
		printf("ww local_ip:%s\n", CONFIG.local_ip);
...
LoadConfiguration(const char *fname)
{
	...
	while (1) {
		...
		if (fgets(optstr, MAX_OPTLINE_LEN - 1, fp) == NULL)
			break;
	...
	if (CONFIG.sndbuf_size == -1)
		 CONFIG.sndbuf_size = sndbuf_size_auto_match;
